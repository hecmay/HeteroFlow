...
extern "C" {
void ultranet(my_ap_axis *in, my_ap_axis *out) {

#pragma HLS INTERFACE m_axi port=in offset=slave bundle=gmem_A
#pragma HLS INTERFACE m_axi port=out offset=slave bundle=gmem_B
...
#pragma HLS ARRAY_PARTITION variable = conv_8_w complete dim = 1
    ...
    hls::stream<my_ap_axis > in_stream;
    #pragma HLS stream variable=in_stream depth=128
    hls::stream<my_ap_axis > out_stream;
    #pragma HLS stream variable=out_stream depth=128    

    for (int i = 0; i < 320 * 640 * 3 * 8 / 64; i++) {
        in_stream.write(in[i]);
    }

    do_compute(in_stream, out_stream, 1);

    for (int i = 0; i < 20 * 40 * 36 / 64; i++) {
