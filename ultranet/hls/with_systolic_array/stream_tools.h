...
void ExtractPixels(stream<my_ap_axis> &in, stream<ap_uint<OutStreamW> > &out,
                   const unsigned reps = 1) {
    my_ap_axis temp;
    // R: NumLines = 640 x 360 x 3 x 8 / 128
    // A: NumLines = 640 x 360 x 3 x 4 / 64
    for (unsigned rep = 0; rep < reps * NumLines; rep++) {
        #pragma HLS loop_tripcount min=20 max=640
...
void adjust_width(stream<ap_uint<IN_BIT> > &in, stream<ap_uint<OUT_BIT> > &out,
    ...
    if (IN_BIT > OUT_BIT) {
        // if the output bitwidth is smaller than the input (减小位宽)
...
#pragma HLS loop_tripcount min=1 max=2
        ...
        // same bitwdith for both input and output (位宽不变)
...
#pragma HLS loop_tripcount min=20 max=640
        ...
        // if the output bitwidth is larger than the input (增大位宽)
...
/**
 * 向流中写入指定数量的0
 * wirte a certain number of zeros into the stream
 */
 ...
 * move a certain amount of packets from one stream to another
 */
template <unsigned IN_BIT>
 ...
 * 多路选择器
 * 将一路输入输出到三路中的一路
 * streaming de-multiplexer
 * stream the input to one of the three outputs
 */
 ...
 * 2路选择器 并且调整数据位宽
 * 将一路输入 输出到两路中的一路
 * 2-way streaming de-multiplexer
 * stream the input to one of the two outputs
 */
...
/**
 * 从内存中读取 nums个数到 流中
 * read "nums" of data packets from memory and write into a stream
 */
