            ...
            unsigned int size) 
{
    ...
    for (unsigned int i=0; i<size; ++i) {
        float error = std::abs(sw_results[i]-hw_results[i]);
...
//    unsigned int num_of_points = 16384 * 16; // 131072 bytes / 2 dimensions / 4 bytes * 16 = 2048 KB
    unsigned int num_of_points = 265216 * 8 * 2; // 2058KB to be evenly divisible by 14 kernels
    //reducing the test data capacity to run faster in emulation mode
    if (xcl::is_emulation()) {
        num_of_points = 265216 * 8 * 2;
    }
    ...
    OCL_CHECK(err, err = merge_krnl.setArg(28, buffer_kNN_hw)); 
    OCL_CHECK(err, err = merge_krnl.setArg(29, buffer_kNN_hw_id)); 
